...
function setup() {
  ...
  Logger.log('Excluded senders loaded: ' + JSON.stringify(excludedSenders));
}
...
function processHelpdeskInbox() {
  ...
  const excludedSenders = loadExcludedSenderAddresses_();
  const query = buildHelpdeskSearchQuery_(excludedSenders);
...
function buildHelpdeskSearchQuery_(excludedSenders) {
  return [
    `to:${HELPDESK_ADDRESS}`,
    'in:inbox',
    '-label:' + PROCESSING_LABEL,
    '-from:' + HELPDESK_ADDRESS,
    ...excludedSenders.map((address) => '-from:' + address),
    'newer_than:7d',
  ].join(' ');
}
...
function processThread_(thread, label, alias, excludedSenders) {
  ...
  if (threadHasExcludedSender_(messages, excludedSenders)) {
    thread.addLabel(label);
...
function threadHasExcludedSender_(messages, excludedSenders) {
  return messages.some((message) => {
    const senderEmail = extractSenderEmail_(message.getFrom());
    return isExcludedSender_(senderEmail, excludedSenders);
  });
}
...
function diagnostics() {
  ...
  const result = {
    ...
    excludedSenders,
    excludedSendersFileId: EXCLUDED_SENDERS_FILE_ID,
