[CmdletBinding()]
param(
    [string]$SourceRoot = "C:\Users\PASCA\Documents",
    [string]$DestinationRoot = "G:\My Drive\PG_Backups\local_documents_code",
    [switch]$DryRun
)

$ErrorActionPreference = "Stop"

$systemName = "local_documents_code"
$timestamp = Get-Date -Format "yyyyMMddTHHmmss"
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$stagingRoot = Join-Path $env:TEMP "pg_portable_backup\staging\local_documents_code"
$logsRoot = Join-Path $scriptDir "logs\local_documents_code"
...
foreach ($file in $files) {
    ...
    $payloadRelative = Join-Path "C\Users\PASCA\Documents" $relative
...
$metadata = [ordered]@{
    ...
    intended_restore_root = "C:\Users\PASCA\Documents"
    ...
    notes = @(
        "This archive is for disaster recovery of the local Codex/code tree under C:\Users\PASCA\Documents.",
        "Files are stored under payload/C/Users/PASCA/Documents to make the original Windows path obvious.",
...
$restoreReadme = @"
# Restore Map - local_documents_code
...
```
C:\Users\PASCA\Documents
```

The archive stores files under:

```
payload\C\Users\PASCA\Documents
```
