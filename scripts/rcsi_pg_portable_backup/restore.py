...
_ARCHIVE_RE = re.compile(r"^(?P<system>[A-Za-z0-9_.\-]+)_(?P<ts>\d{8}T\d{6})\.tar\.gz$")
...
def _pick_archive(cfg: dict, selector: str) -> str:
    ...
    if not items:
        raise RuntimeError("no archives found on remote for system " f"{cfg.get('system_name')!r}")
        ...
        for p in sorted(src.rglob("*")):
            try:
                if p.is_file():
                    ...
                    if rel.parts and rel.parts[0] in ("pg_portable_backup", "staging", "logs"):
                        continue
...
def run(cfg: dict, archive_file: Path | None, selector: str,
    ...
    with open(log_path, "w", encoding="utf-8") as log:
        ...
        try:
            ...
            say(
                f"archive system={manifest.get('system_name')} "
            ...
            if restore_mode == "fast":
                ...
                selected_mode = "fast" if has_fast_snapshot and cluster_cfg.get("data_dir") else "portable"
            ...
            if selected_mode == "fast":
                ...
                say(f"pg_restore -> database {cfg['postgres']['database']} (drop_first={drop})")
                pg.restore_database(cfg["postgres"], dump, cfg["postgres"]["database"], log_fp=log, drop_first=drop)
                ...
                if not app_stage.exists():
                    ...
                    for item in planned_targets:
                        ...
                        if not dst_str or dst_str == "(no target configured)":
                            say(f"  SKIP {label}: no target (missing _sources.json and source_root)")
                            continue
                        dst = Path(dst_str)
            ...
            if not skip_env:
                ...
                if env_enc.exists() and env_dst_str:
                    ...
                    if not key_path.exists():
                        say(f"WARNING: env key missing at {key_path} - .env NOT decrypted. Copy env_key.bin from the source machine and re-run restore.py --skip-app")
                    else:
            ...
            say("=== DONE OK ===")
            say("Next: start your application, confirm it connects, and spot-check a few tables.")
            return 0
...
def main() -> int:
    ...
    ap.add_argument("--config", help="Path to config JSON (default: config.<system>.json next to this script)")
    g = ap.add_mutually_exclusive_group()
    g.add_argument("--install", help="Install this archive. 'latest' picks the newest from the remote; or pass a YYYYMMDDTHHMMSS timestamp or full filename.")
    g.add_argument("--install-file", help="Use a local .tar.gz instead of downloading from the remote.")
    ap.add_argument("--list", action="store_true", help="Just list remote archives and exit.")
    ap.add_argument("--drop", action="store_true", help="DROP the target database first if it exists.")
    ap.add_argument("--restore-mode", choices=["auto", "fast", "portable"], default="auto",
                    help="Choose fast physical restore, portable logical restore, or auto-select based on archive contents.")
    ap.add_argument("--dry-run", action="store_true", help="Verify and preview the restore without changing the database, app files, or env/config.")
    ap.add_argument("--skip-app", action="store_true", help="Do not overwrite source_root app files.")
    ap.add_argument("--skip-env", action="store_true", help="Do not decrypt env.")
    ap.add_argument("-y", "--yes", action="store_true", help="Skip the typed RESTORE confirmation.")
    args = ap.parse_args()
    ...
    if not args.install and not args.install_file:
        print("Pass either --install <ts|latest> or --install-file <path>, or --list to see what's on the remote.", file=sys.stderr)
        return 2
